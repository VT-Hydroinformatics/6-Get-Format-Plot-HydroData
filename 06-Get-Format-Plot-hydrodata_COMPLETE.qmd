...
Remember, all the parameter codes and site names get passed to dataRetrieval functions as characters, so they must be in quotes and with "USGS-" at the front.
...
If you look at dataAvailable, you'll see it contains the USGS site number, but not the name of the site. This makes it challenging to interpret if you a human who prefers words to long sequences of numbers.

Site name is in the siteinfo object, and it's easy enough to look there for one site. However, we often have many sites, so we need a better way to add this information to our data. 
...
#you can get all the site in VA using read_waterdata_monitoring_location as follows
...
First, we will use select to trim the data to just what we need, then call pivot_wider telling it which data to use for the new column names (names_from = monitoring_location_name) and what values we want to pivot into the data under those columns (values_from = values). We will have to filter to just discharge first as well. We will do this before selecting the columns and pivoting.
...
Now, let's clean things up a bit. Hang on tight, we are going to do a bunch of data wrangling in one pipe sequence to accomplish what we want: A dataset that has a chloride and specific conductance in their own columns, with a column for time, and one that says what site the measurement was from.
...
So! We will pivot longer, teling pivot_longer() to pivot the columnd Spec_Cond and Chloride.
